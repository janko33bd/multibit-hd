 ...
 *
 */
public class CryptoCoinsDecorator {

  public static final Font CRYPTOCOINS_FONT;

  static {

    // See FontAwesomeTools when upgrading to later version
    try (InputStream in = CryptoCoinsDecorator.class.getResourceAsStream("/assets/fonts/cryptocoins.ttf")) {

      Font font = Font.createFont(Font.TRUETYPE_FONT, in);
      Preconditions.checkNotNull(font, "Font Cryptocoins not loaded");

      CRYPTOCOINS_FONT = font.deriveFont(Font.PLAIN, MultiBitUI.NORMAL_ICON_SIZE);
   ...
   *
   * @param CryptoCoinsIcon The awesome icon reference
   * @param color       The icon color
   * @param size        The icon size (font metrics)
   *
   * @return The icon
   */
  public static Icon createIcon(CryptoCoinsIcon CryptoCoinsIcon, Color color, int size) {

    JLabel iconLabel = new JLabel();
    iconLabel.setFont(iconLabel.getFont().deriveFont((float) size));
    iconLabel.setForeground(color);

    return new CryptocoinsSwingIcon(iconLabel, CryptoCoinsIcon.getChar());
  }
   ...
   * @param size    The icon size (font metrics)
   */
  public static void applyIcon(CryptoCoinsIcon icon, JLabel label, boolean leading, int size) {
    ...
    Icon enabledIcon = new CryptocoinsSwingIcon(iconLabel, icon.getChar(), true);
    Icon disabledIcon = new CryptocoinsSwingIcon(iconLabel, icon.getChar(), false);
   ...
   * @param size    The icon size (font metrics)
   */
  public static void bindIcon(CryptoCoinsIcon icon, JLabel label, boolean leading, int size) {

    Icon enabledIcon = new CryptocoinsSwingIcon(label, icon.getChar(), size, true);
    Icon disabledIcon = new CryptocoinsSwingIcon(label, icon.getChar(), size, false);
  ...
  public static void applyIcon(CryptoCoinsIcon icon, JButton button, boolean leading, int size) {
  ...
  public static void applyIcon(CryptoCoinsIcon icon, JButton button, boolean leading, int verticalAlignment, int size) {

    JButton iconButton = new JButton();
    iconButton.setFont(iconButton.getFont().deriveFont((float) size));
    iconButton.setForeground(button.getForeground());

    Icon enabledIcon = new CryptocoinsSwingIcon(iconButton, icon.getChar(), true);
    Icon disabledIcon = new CryptocoinsSwingIcon(iconButton, icon.getChar(), false);
   ...
   * @param size    The icon size (font metrics)
   */
  public static void bindIcon(CryptoCoinsIcon icon, JButton button, boolean leading, int size) {

    Icon enabledIcon = new CryptocoinsSwingIcon(button, icon.getChar(), size, true);
    Icon disabledIcon = new CryptocoinsSwingIcon(button, icon.getChar(), size, false);
   ...
   */
  public static CryptoCoinsIcon select(CryptoCoinsIcon ltrIcon, CryptoCoinsIcon rtlIcon) {
